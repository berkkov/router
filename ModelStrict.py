...
'''
K_ARRAY = [[1, 2, 3, 4, 5, 6],
           [[1, 2], [1, 3], [1, 4], [1, 5], [1, 6], [2, 3], [2, 4], [2, 5], [2, 6], [3, 4], [3, 5], [4, 5], [3, 6],
            [4, 6], [5, 6]],
           [[1, 2, 3], [1, 2, 4], [1, 2, 5], [1, 2, 6], [1, 3, 4], [1, 3, 5], [1, 4, 5], [1, 3, 6], [1, 4, 6],
            [1, 5, 6], [2, 3, 4], [2, 3, 5],
            [2, 3, 6], [2, 4, 5], [2, 4, 6], [2, 5, 6], [3, 4, 5], [3, 4, 6], [3, 5, 6]],
...
class problem(object):
    ...
    def create_model(self):
        ...
        self.sigma = pulp.LpVariable.dicts(name='sigma', indexs=self.periods, lowBound=0, upBound=1,
                                           cat=pulp.LpInteger)  #
        ...
        self.model += pulp.lpSum(
            [self.x[(i, j, p)] * self.durations[i][j] for i in range(self.n_with_dummies) for j in
        ...
        for i in range(1, self.n + 1):
            ...
            for p in self.periods:
                ...
                self.model += (pulp.lpSum(
                    [[self.x[(i, j, p)]] for j in (s for s in range(1, self.n + 1) if s != i)])) == (
                                  pulp.lpSum([self.y[(i, k)] for k in k_pos]))
        ...
        for i in range(1, self.n + 1):
            self.model += pulp.lpSum([self.x[(i, j, p)] for p in self.periods for j in
                                      (s for s in range(1, self.n_with_dummies) if s != i)]) == self.frequencies[
                              i]
    ...
    def solve(self):
        ...
        :return:
        '''
        self.status = self.model.solve(pulp.GLPK_CMD(options=['--tmlim', '600'], keepFiles=False))
